 ...
 * público. Fue el backend completo de la app; desde v6 le queda UNA sola
 * capacidad, `notifyFotoPending`, porque `MailApp` no tiene equivalente en el
 * SDK de Google APIs. Todo lo demás —leer y escribir la planilla, subir y mover
 * archivos en Drive, provisionar la instancia— lo hace la app directo con una
 * service account (ver lib/google/).
 *
 * Lo que corre acá se ejecuta con TU cuenta, la que desplegó el script.
 *
 * --- CÓMO DESPLEGAR ---
 *
 ...
 *    (Sheets, para leer la lista de destinatarios en la hoja "Config", y envío
 *    de correo). Si aparece "Esta app no está verificada" haz click en
 ...
 *
 * 9. Provisión: YA NO SE HACE ACÁ. Desde v6 el script no atiende `setup`; las
 *    hojas y el árbol de carpetas los crea la app con la service account:
 *
 *      curl -s -X POST 'http://localhost:3000/api/migracion/setup?rootFolderId=<ID>'
 *
 *    Se corre desde local contra la planilla real, con las credenciales de
 *    `.env.local`. Ver ARQUITECTURA.md → "Puesta en marcha de una instancia".
 *
 * --- ACTUALIZACIONES ---
 *
...
// ----- SUPERFICIE EXPUESTA -----------------------------------------------
//
// Este endpoint es público y sin autenticación: "Ejecutar como: yo" + "Quién tiene
// acceso: cualquier usuario" significa que quien tenga la URL opera con la cuenta que
// desplegó el script. La URL viajó en el JS de un frontend público, así que hay que
// tratarla como conocida.
//
// El script atiende SOLO lo que la app no puede hacer por el SDK de Google (ver
// lib/google/actions.js). Todo lo demás se retiró: no porque estuviera roto, sino
// porque cada action viva es una capacidad que cualquiera en internet puede
// ejecutar. v5 se llevó las 8 lecturas —que exponían la planilla completa— y todas
// las escrituras a Sheets. v6 se lleva las cuatro de Workspace, que dejaron de
// depender de algo externo cuando las carpetas de la app pasaron a una Unidad
// compartida (2026-08-06): adentro los archivos son de la unidad y no de quien los
// crea, así que la falta de cuota de la service account dejó de importar.
//
// Queda una capacidad, y no toca ningún dato:
//
//   notifyFotoPending   manda el correo de "hay una foto pendiente". `MailApp` no
//                       tiene equivalente en el SDK, y las alternativas (API de
//                       Gmail con delegación de dominio, o un proveedor tipo
//                       Resend) cuestan más que el beneficio para una sola
//                       notificación. Por eso APPS_SCRIPT_URL sigue siendo
//                       requerida y este /exec no se puede dar de baja.
//   ping                versión desplegada; lo consulta /api/health.
//
// `setup` fue la última en salir, y era la más peligrosa de las seis: escribe la
// clave `driveFolders` de la hoja "Config" con los IDs del árbol que cuelga del
// `rootFolderId` que le pasen. Cualquiera con la URL podía apuntarla a una carpeta
// propia y la app habría empezado a subir los documentos ahí. Hoy la provisión se
// corre desde local con la service account: POST /api/migracion/setup.
//
// Al retirar una action más, sacarla de acá y de su rama en doGet/doPost.
var ACTIONS_ACTIVAS = {
  ping: true,
  notifyFotoPending: true,
};

// Casi todo el archivo es ya código inalcanzable por HTTP. Lo que corre de verdad
// cabe en cuatro funciones: doGet → ping, y doPost → notifyFotoPending →
// getConfigValue → rcSpreadsheet.
//
// Y aun así NO se borra el resto. Este script convive con otros archivos en el mismo
// proyecto de Apps Script (el procesador de Combustible, que es de dónde viene el
// choque de nombres de `WEB_CFG`, más abajo), y esos archivos pueden llamar a
// cualquiera de estas funciones sin que se vea desde acá. Borrarlas es una limpieza
// que se hace en el editor, mirando los otros archivos, no desde el repo. Lo que
// importa —que nadie las pueda ejecutar por la URL— ya está.
function actionRetirada(action) {
  return jsonOut({
    error:
      "action retirada de este backend: " + action + ". La sirve el SDK de Google " +
      "desde la app (ver RC_SDK_ACTIONS). Script " + SCRIPT_VERSION + ".",
  });
}
...
function doGet(e) {
  try {
    // Sin `action` ya NO se asume "read": ese default hacía que un GET pelado a la URL
    // devolviera la planilla completa. Ahora no devuelve nada.
...
//
// Desde v6 ningún camino HTTP lo usa —lo único que queda manda un correo—, pero las
// funciones que se corren a mano desde el editor sí, y el SDK no tiene lock: lo que
// protege del clobber allá es escribir por clave, no serializar. Ver ARQUITECTURA.md.
...
function doPost(e) {
  try {
    ...
    // Sin lock: mandar un correo no toca la planilla más que para leer la lista de
    // destinatarios, y no hay carrera posible entre dos envíos.
...
// RETIRADA EN v6: no es alcanzable por HTTP. La provisión la corre la app con la
// service account — POST /api/migracion/setup?rootFolderId=<ID>, desde local. La
// traducción vive en `setup` de lib/google/actions.js, verificada contra esta
// función con /api/migracion/probe-setup.
//
// Queda acá porque se puede ejecutar a mano desde el editor de Apps Script (Run ▶),
// que es la única vuelta atrás si la provisión por SDK fallara a mitad de camino.
//
