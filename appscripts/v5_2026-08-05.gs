...
//
// Desde v5 el script atiende SOLO lo que la app todavía no puede hacer por el SDK de
// Google (ver lib/google/actions.js). Todo lo demás se retiró: no porque estuviera
// roto, sino porque cada action viva es una capacidad que cualquiera en internet
// puede ejecutar. Se fueron las 8 lecturas —que exponían la planilla completa— y
// todas las escrituras a Sheets, incluidas `setSheetRows` y `setEmissions`, que
// borraban y reescribían hojas enteras.
//
// Lo que queda no puede tocar los datos de consumo:
//
//   upload · move · deleteFile   archivos en Drive. Una service account no tiene
//                                cuota propia en Drive, así que esto necesita Unidad
//                                compartida (Workspace) o delegación de dominio.
//   notifyFotoPending            MailApp no existe en el SDK.
//   setup                        crea el árbol de carpetas de Drive, así que depende
//                                de lo mismo. (`init`, que solo crea hojas, ya está
//                                en el SDK y salió de acá.)
...
// Las funciones de Sheets siguen más abajo en el archivo y NO se borraron: `setup`
// depende de varias (ensureSheets, setConfigValue), y el grafo de dependencias no se
// puede verificar sin correr el script. Ya no son alcanzables por HTTP, que es lo que
// importa. Borrarlas es una limpieza posterior, hecha desde el editor donde se puede
