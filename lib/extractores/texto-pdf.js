...
export async function textoDePdf(buffer) {
  ...
  const pdf = await getDocument({
    ...
    // Antes acá iban `disableWorker: true` e `isEvalSupported: false`: ninguna de
    // las dos existe en pdfjs 6, así que no hacían nada. En Node pdfjs siempre
    // levanta su "fake worker" (el worker corriendo en el mismo hilo), que carga
    // pdf.worker.mjs con un import dinámico — de ahí el outputFileTracingIncludes
    // de next.config.mjs, sin el cual el archivo no viaja en la función.
  }).promise;
