// Parsers de boletas por proveedor. Portados VERBATIM de proto/extractors.jsx,
// que a su vez replican `lector_automático_facturas_(1).ipynb`.
//
// Estas expresiones regulares están ajustadas contra boletas reales de cada
// proveedor: se conservan tal cual, incluidos los nombres originales de las
// funciones, porque cualquier "mejora" cosmética arriesga romper la extracción de
// un formato que no se puede volver a probar sin la boleta que lo motivó.
//
// Son funciones puras sobre el texto ya extraído del PDF: no tocan pdfjs ni el
// DOM, así que corren igual en el servidor.
...
export function rcPeriodoMedio(sIni, sFin) {
  const parse = (s) => {
...
export function rcExtraerEnel(textBundle) {
  ...
  if (/No Facturado/i.test(texto)) {
    ...
    // CORRECCIÓN respecto del prototipo. Los patrones eran `(\d+)`, sin admitir
    // el punto de miles, así que en "Electricidad Consumida ( 20.440 kWh )" el
    // primer patrón no calzaba y el fallback `/(\d+)\s*kWh/` leía **440**: un
    // consumo 46 veces menor, sin ningún aviso. El parser de CGE ya lo hacía
    // bien —`([\d.]+)` y quitar los puntos— y su propio comentario documenta que
    // las boletas imprimen los miles con punto. Se aplica el mismo tratamiento.
    //
    // Efecto lateral asumido: un valor con decimales tipo "12,5 kWh" no cambia
    // (la coma no entra en la clase), pero "12.5 kWh" se leería como 125. En
    // consumos eléctricos, que se facturan en kWh enteros, el punto es separador
    // de miles.
    const aEntero = (s) => parseInt(s.replace(/\./g, ""), 10) || 0;
    const mCons = texto.match(/Electricidad Consumida\s*\(?\s*([\d.]+)\s*kWh/i);
    if (mCons) out.consumo = aEntero(mCons[1]);
    else {
      const alt = texto.match(/([\d.]+)\s*kWh/);
      if (alt) out.consumo = aEntero(alt[1]);
    }
  }
...
export function rcExtraerCGE(textBundle) {
  const texto = textBundle.combined;
...
export const RC_MESES_CL = { ENE:"01", FEB:"02", MAR:"03", ABR:"04", MAY:"05", JUN:"06",
                      JUL:"07", AGO:"08", SEP:"09", OCT:"10", NOV:"11", DIC:"12" };
// "28-MAY-2025" → "28/05/2025" (formato que espera rcPeriodoMedio)
export function rcFechaMesNombre(dia, mesStr, anio) {
  return `${dia}/${RC_MESES_CL[mesStr.toUpperCase()] || "01"}/${anio}`;
}

export function rcExtraerAguas(textBundle) {
  const texto = textBundle.combined;
  const out = { numeroCliente: "", fecha: "", periodoInicio: "", periodoFin: "", consumo: 0, costo: 0 };
...
export function rcExtraerAguasDelValle(textBundle) {
  const texto = textBundle.combined;
  const out = { numeroCliente: "", fecha: "", periodoInicio: "", periodoFin: "", consumo: 0, costo: 0 };
...
export function rcExtraerEsval(textBundle) {
  const texto = textBundle.combined;
  const out = { numeroCliente: "", fecha: "", periodoInicio: "", periodoFin: "", consumo: 0, costo: 0 };
...
export function rcFechaTextoCL(s) {
...
export function rcExtraerChilquinta(textBundle) {
  const texto = textBundle.combined;
  const out = { numeroCliente: "", fecha: "", periodoInicio: "", periodoFin: "", consumo: 0, costo: 0 };
