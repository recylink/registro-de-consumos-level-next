...
// El .xlsx se abre en Excel, donde un texto que empieza con = es una fórmula
// ejecutable (=cmd|… corre comandos). Mismo saneo que en las escrituras a Sheets.
import { sanearFilas } from "../domain/sanear";
import {
  consumoFor,
  costoFor,
  MED_TYPES,
...
export function medidoresWorkbook({ M, records, sucursal, tipo, meses }) {
  ...
  const hojaDetalle = XLSX.utils.aoa_to_sheet(sanearFilas(detalle));
  ...
  const hojaTotales = XLSX.utils.aoa_to_sheet(sanearFilas(totales));
