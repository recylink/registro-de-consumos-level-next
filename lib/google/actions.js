...
// Copiados TAL CUAL de RC_PROVIDERS y RC_RESPALDO_TYPES (apps-script.gs:542 y 554),
// incluido "ESVAL" en mayúsculas.
//
// No se toman de PROVEEDORES_DOC (lib/domain/proveedores.js) aunque sea el catálogo
// de la app, porque las listas NO son iguales: ahí "Esval" va capitalizado y además
// están `chilquinta` y `generic`. Usar ese catálogo crearía carpetas nuevas al lado
// de las que ya existen —"Esval" junto a "ESVAL"— y `setup` dejaría de ser
// idempotente sobre las instancias ya provisionadas, que es su única promesa.
//
// La divergencia queda anotada, no arreglada acá: `chilquinta` no tiene carpeta
// propia, así que sus documentos caen en `uploadFacturas` (ver providerFolders en
// lib/drive-folders.js). Emparejar las listas es un cambio de datos, no de backend.
