import "server-only";
import { HOJAS_REGISTROS, SHEETS, spreadsheetId, spreadsheetUrl } from "../instance";
import {
  ENCABEZADOS,
  ENCABEZADOS_CONFIG,
 ...
 *
 * `setup` la llama como primer paso, igual que el original: provisionar es crear las
 * hojas y después el árbol de carpetas.
 */
async function init() {
...
/**
 * uploadFile (apps-script.gs:261). Sube un archivo, creando las subcarpetas que
 * falten de la ruta pedida.
 *
 * El base64 llega del Server Action, que ya convirtió el File del navegador
 * (lib/drive.js). Se mantiene ese contrato aunque el SDK podría recibir el stream
 * directo: cambiarlo obligaría a tocar los cinco llamadores y a que dejaran de ser
 * agnósticos del backend, que es lo que hace reversible cada paso de la migración.
 *
 * Los defaults de nombre y mimeType son los del original, incluido el "archivo" sin
 * extensión: cambiarlos acá haría que el mismo upload guarde distinto según qué
 * backend esté habilitado.
 */
async function upload({ name, mimeType, base64, folderId, subfolders }) {
  if (!folderId) throw new Error("folderId missing");
  const destino = await asegurarSubcarpetas(folderId, subfolders || []);
  return crearArchivo({
    nombre: name || "archivo",
    mimeType: mimeType || "application/octet-stream",
    contenido: Buffer.from(base64 || "", "base64"),
    carpeta: destino,
  });
}
...
// ----- Provisión con Drive (bloque G) -------------------------------------

// Proveedores con carpeta dedicada, y tipos de consumo con carpeta de respaldos.
// Vienen de RC_PROVIDERS y RC_RESPALDO_TYPES del .gs (apps-script.gs:542 y 554).
//
// **Los nombres son datos, no etiquetas.** Cada uno es el nombre de una carpeta que ya
// existe en Drive, y `asegurarSubcarpetas` la busca por nombre. Cambiar uno acá no
// renombra nada: crea una carpeta nueva al lado de la vieja, y los documentos empiezan
// a caer en la nueva mientras los viejos quedan en la otra. Por eso "ESVAL" sigue en
// mayúsculas aunque el catálogo de la app escriba "Esval" (PROVEEDORES_DOC, en
// lib/domain/proveedores.js): la carpeta se llama así. Vale al revés también —
// renombrar la carpeta en Drive sin tocar esta lista hace que el próximo `setup`
// recree la vieja.
//
// Por eso esta lista NO se deriva de PROVEEDORES_DOC, aunque sea el catálogo. Son dos
// cosas distintas: ahí está lo que la app ofrece elegir; acá, cómo se llaman las
// carpetas que existen.
//
// `chilquinta` se agregó el 2026-08-10, después de correr `setup` con `forzar`: tiene
// extractor propio y está visible en la app, así que sus documentos merecían carpeta
// en vez de caer en la bolsa genérica.
//
// `generic` queda afuera a propósito, y no es un olvido: es "Otro proveedor", oculto y
// sin extractor. `uploadFacturas` ("Documentos subidos") ES su carpeta — es lo que
// `providerFolders` (lib/drive-folders.js) devuelve cuando no hay par configurado.
// Darle una propia sería partir la bolsa genérica en dos.
const PROVEEDORES_CON_CARPETA = [
  ["enel", "Enel"],
  ["cge", "CGE"],
...
/**
 * setupInstance (apps-script.gs:561). Provisión de una instancia: crea las hojas y
 * el árbol de ~25 carpetas bajo la raíz que se le pase, y deja los IDs en la clave
 * `driveFolders` de la hoja "Config".
 *
 * Idempotente, como el original: cada carpeta se busca por nombre antes de crearse
 * (`asegurarSubcarpetas`), así que correrlo dos veces no duplica nada ni cambia
 * ningún ID.
 *
 * El original corría entero adentro de `withLock` (apps-script.gs:181). Acá no hay
 * lock, y no hace falta por lo mismo que en `upload`: la creación de cada carpeta
 * converge a la más vieja si dos procesos compiten. Igual, `setup` lo corre una
 * persona a mano una vez por instancia, no es un camino concurrente.
 *
 * NO lo llama ninguna pantalla: se corre una vez por instancia desde
 * `POST /api/migracion/setup`, a mano y desde local. Por eso la respuesta conserva la
 * forma del original, que es para leer.
 */
async function setup({ rootFolderId }) {
  await init();
  if (!rootFolderId) {
    throw new Error("rootFolderId missing: pasa el ID de la carpeta raíz de Drive.");
  }

  // Una carpeta por nombre bajo un padre. Es `childFolder` (apps-script.gs:556).
  const hija = (padre, nombre) => asegurarSubcarpetas(padre, [nombre]);

  const fotos = await hija(rootFolderId, "Fotos");
  const facturas = await hija(rootFolderId, "Facturas");
  const medidores = await hija(rootFolderId, "Medidores");
  const respaldos = await hija(medidores, "Respaldos");
  const proveedores = await hija(rootFolderId, "Proveedores");

  const folders = {
    fotosPorCompletar: await hija(fotos, "Por completar"),
    fotosProcesados: await hija(fotos, "Procesados"),
    manualFacturas: await hija(facturas, "Registro manual"),
    uploadFacturas: await hija(facturas, "Documentos subidos"),
    medidorFacturas: await hija(medidores, "Facturas"),
    medidorPagos: await hija(medidores, "Pagos"),
    medidorRespaldos: {},
    proveedores: {},
  };

  for (const [tipo, nombre] of TIPOS_CON_RESPALDO) {
    folders.medidorRespaldos[tipo] = await hija(respaldos, nombre);
  }

  for (const [id, nombre] of PROVEEDORES_CON_CARPETA) {
    const dir = await hija(proveedores, nombre);
    folders.proveedores[id] = {
      porProcesar: await hija(dir, "Por procesar"),
      procesados: await hija(dir, "Procesados"),
    };
  }

  // ÚLTIMO, igual que el original: si algo falla creando carpetas, la config vieja
  // queda intacta en vez de quedar a medio escribir.
  await setConfig({ key: "driveFolders", value: folders });

  return {
    ok: true,
    // El original devolvía SCRIPT_VERSION, la versión del .gs desplegado. Acá no
    // corre ningún script: se dice de dónde salió la respuesta, que es la
    // pregunta que esa clave contestaba.
    backend: "sdk",
    spreadsheetId: spreadsheetId(),
    spreadsheetUrl: spreadsheetUrl(),
    rootFolder: await infoCarpeta(rootFolderId),
    folders,
  };
}

// ----- Registro -----------------------------------------------------------
// La migración está completa salvo `notifyFotoPending`, que se queda en el Apps
// Script porque `MailApp` no tiene equivalente en el SDK. Script `v6` = estas dos
// actions y nada más: `notifyFotoPending` y `ping`.
//
