...
import { neutralizarFormula, sanearFilas } from "../domain/sanear";
...
export async function hojas({ ahora = 0 } = {}) {
  ...
  cacheHojas = (res.data.sheets || []).map((s) => ({
    ...
    // Por la misma razón, del otro lado: escribir en una columna que la grilla no
    // tiene es un 400, no una extensión automática. Lo usa `asegurarAncho`.
 ...
 *
 * La contra de USER_ENTERED es que un texto que empieza con = + - @ queda como
 * fórmula ejecutable, así que TODO lo que se escribe pasa antes por
 * `neutralizarFormula` (lib/domain/sanear.js). Los números y las fechas siguen
 * intactos, que es lo único que este modo tiene que interpretar.
 */
...
export async function agregarFilas(nombre, filas, { modo = MODO_ESCRITURA } = {}) {
  ...
  await sheetsApi().spreadsheets.values.append({
    ...
    requestBody: { values: sanearFilas(filas) },
  });
  return filas.length;
}
 ...
 * falta escribir por filas — `escribirFilas` + `upsertPorClave` (actions.js).
 *
 * Por eso esta función es la excepción y no la regla. Sirve donde la hoja tiene un
 * solo escritor y se reescribe entera por diseño (config, emisiones); NO sirve para
 * hojas que se editan celda por celda desde la UI. El módulo Medidores usaba esto
 * y era la fuente concreta de pérdida de datos: ver lib/domain/medidores-patch.js.
 *
  ...
  if (contenido.length) {
    ...
    await conEspera(() =>
      sheetsApi().spreadsheets.values.update({
        ...
        requestBody: { values: sanearFilas(contenido) },
      }),
    );
  }
...
/**
 * Borra filas por índice. `indices` son 0-based sobre la grilla, contando el
 * encabezado como 0 — los mismos que devuelve `leerHoja`.
 *
 * Va por `spreadsheets.batchUpdate` y no por la API de valores, porque borrar una
 * fila es un cambio de estructura: la API de valores solo sabe vaciar celdas, y
 * vaciarlas dejaría huecos donde el Apps Script quitaba la fila entera.
 *
 * Los índices se ordenan de mayor a menor. batchUpdate aplica sus requests EN
 * SECUENCIA, así que borrar de arriba hacia abajo correría los índices de los que
 * faltan y terminaría borrando filas equivocadas. Es la misma razón por la que el
 * bucle del Apps Script iba al revés (apps-script.gs:387).
 */
export async function borrarFilas(nombre, indices) {
  const orden = [...new Set(indices || [])].filter((i) => i >= 1).sort((a, b) => b - a);
  if (!orden.length) return 0;
  const sheetId = await sheetIdDe(nombre);
  if (sheetId == null) throw new Error("sheet not found: " + nombre);
  contador.escrituras++;
  await conEspera(() =>
    sheetsApi().spreadsheets.batchUpdate({
      spreadsheetId: spreadsheetId(),
      requestBody: {
        requests: orden.map((i) => ({
          deleteDimension: {
...
/**
 * Se asegura de que la grilla tenga al menos `columnas` columnas. Devuelve cuántas
 * agregó.
 *
 * Escribir en una columna que la grilla no tiene NO la crea: la API responde 400
 * "exceeds grid limits". Una hoja recién creada trae 26 columnas, así que el caso
 * normal es que no haga falta nada; pero una hoja recortada a mano puede tener
 * exactamente las de su encabezado, y ahí agregar una columna nueva falla.
 */
export async function asegurarAncho(nombre, columnas) {
  const meta = (await hojas()).find((h) => h.titulo === nombre);
  if (!meta) throw new Error("sheet not found: " + nombre);
  const faltan = columnas - meta.columnas;
  ...
  await conEspera(() =>
    sheetsApi().spreadsheets.batchUpdate({
      ...
      requestBody: {
        requests: [
          {
            appendDimension: { sheetId: meta.sheetId, dimension: "COLUMNS", length: faltan },
          },
        ],
      },
    }),
  ...
  await sheetsApi().spreadsheets.values.batchUpdate({
    ...
    requestBody: {
      ...
      data: validas.map((c) => ({
        ...
        values: [[neutralizarFormula(c.value)]],
      })),
    },
  });
...
/**
 * Filas enteras de una misma hoja, en un solo request. `filas`: [{ row, values }]
 * con `row` 1-based.
 *
 * Es la escritura que `reemplazarHoja` no sabe hacer: toca solo los renglones
 * indicados y deja el resto de la hoja como estaba, así que dos clientes que
 * editan filas distintas no se pisan. Ver `upsertPorClave` en actions.js, que es
 * quien decide qué fila le toca a cada clave.
 *
 * Escribe el rango A..N de cada fila en vez de celda por celda a propósito: evita
 * tener que comparar el valor viejo con el nuevo para saber qué celdas cambiaron,
 * y esa comparación es traicionera porque la lectura viene cruda (números) y la
 * escritura va como USER_ENTERED (texto interpretado). Como el rango llega solo
 * hasta el ancho de `values`, una columna que un humano haya agregado a la derecha
 * del encabezado sobrevive intacta.
 */
export async function escribirFilas(nombre, filas, { modo = MODO_ESCRITURA } = {}) {
  const validas = (filas || []).filter((f) => f && f.row >= 1 && f.values && f.values.length);
  if (!validas.length) return 0;
  const hoja = `'${nombre.replace(/'/g, "''")}'`;
  contador.escrituras++;
  await conEspera(() =>
    sheetsApi().spreadsheets.values.batchUpdate({
      ...
      requestBody: {
        valueInputOption: modo,
        data: validas.map((f) => ({
          range: `${hoja}!A${f.row}:${letraColumna(f.values.length)}${f.row}`,
          values: [f.values.map(neutralizarFormula)],
        })),
      },
    }),
  );
