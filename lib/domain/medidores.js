...
import { MED_TYPES, priceFor } from "./medidores-calc";
...
/**
 * Precio unitario de (sucursal, tipo, mes). Vacío lo borra, y el mes vuelve a
 * heredar el precio del mes anterior que tenga uno.
 *
 * Escribir el mismo valor que ya se heredaba no crea registro: el cálculo daría
 * idéntico y solo ensuciaría la hoja con tarifas repetidas — además de apagar la
 * marca de "heredado" en la celda, que es información útil.
 */
export function setPrice(M, { sucursal, type, month, precio }) {
  ...
  if (!vacio(precio)) {
    const heredado = priceFor(prices, sucursal, type, month);
    if (heredado !== Number(precio)) {
      prices.push({ sucursal, type, month, precio: Number(precio) });
    }
  }
  return { ...M, prices };
}
...
/**
 * Mueve el primer precio de (sucursal, tipo) a `month`, para que los meses
 * anteriores que estaban sin tarifa hereden desde ahí.
 *
 * Es la contraparte explícita de haber sacado la herencia hacia atrás: si la
 * tarifa de julio también valía para los meses previos, el usuario lo dice con un
 * botón en vez de que el cálculo lo asuma solo.
 */
export function applyPriceFrom(M, { sucursal, type, month, precio }) {
  if (vacio(precio)) return M;
  const mismos = (p) => p.sucursal === sucursal && p.type === type;
  // Los precios de esta (sucursal, tipo) anteriores o iguales al mes destino
  // quedan cubiertos por el nuevo, que arranca antes que todos ellos.
  const prices = (M.prices || []).filter((p) => !(mismos(p) && p.month <= month));
  prices.push({ sucursal, type, month, precio: Number(precio) });

  // Un precio idéntico al que ya rige no aporta nada: se descartan los tramos
  // repetidos consecutivos para no dejar la hoja llena de tarifas iguales.
  const propios = prices.filter(mismos).sort((a, b) => (a.month < b.month ? -1 : 1));
  const redundantes = new Set();
  let vigente = null;
  for (const p of propios) {
    if (vigente !== null && Number(p.precio) === vigente) redundantes.add(p);
    else vigente = Number(p.precio);
  }
  return { ...M, prices: prices.filter((p) => !redundantes.has(p)) };
}
