...
//
// `refrigerantes` NO está en la lista, y no es un olvido: no se registra como un
// consumo. Un consumo es "gasté 640 kWh en abril"; una recarga de refrigerante es
// "puse 2,5 kg de R-410A en el equipo", sin lectura mensual ni boleta, y con una
// emisión enorme por kilo. Vive en Impacto → Factores de emisión, por sucursal.
//
// Mientras estuvo acá, la app ofrecía configurarlo y después no había dónde
// registrarlo, y la matriz de carga mostraba una columna que nunca podía quedar
// "cargada" porque esos datos no están en `records`.
//
// `emptyItems()` sigue creando la clave, y la capa de planilla
// (lib/sheets/sucursales.js) mantiene su propia lista con los cuatro tipos a
// propósito, para no borrar lo que alguna sucursal ya tenga configurado. El día
// que se registre como un tipo más, esta línea es el interruptor.
